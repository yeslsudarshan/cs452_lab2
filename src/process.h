...
typedef struct PCB {
 ...
 // put in data used for your scheduling algorithm here
  ...
  uint32 	estcpu; 	//The estcpu for the process. Incremented every quantum
  uint32        sleep_timestamp; //time stamp when the process was put to bed(In quantum)
  uint32        wakeup_timestamp; 
  int           p_nice;
} PCB;
