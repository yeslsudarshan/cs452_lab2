...
//Total num of quanta
uint32 total_num_quanta;

//---------------------------------------------------------------------
//  
// DLXOS  Timerget() bug  workaround 
// 
// Workaround suggested for the Timerget() function
//
//----------------------------------------------------------------------
...
//---------------------CS452__4.4 BSD SCHEDULING SPECIFIC--------------------------/


// Decays the estcpu values for all the processes in the queue(Running queue)
void decay_estcpu_prio(Queue* q)
 {
    Link *l;
    int i,j=0,flag=0;
    PCB* pcb;
...
ProcessSchedule ()
{
  ...
  Get the process at the head of the top most non-empty queue.
  If the process' estcpu has elapsed 4 quanta,recalc prio, Put in appropriate queue.
  PRIO = PUSER +(estcpu/4) + 2*p_nice
  Queue = PRIO/4
  Note : First increment estcpu and then calc prio.

  every 10 quanta : 
  If decay estcpu of all the procs in the queue. Recalc prio of all.Reorder all queues.
  Note: Mark the tails of each queue to avoid double check.
  Decay : 
  estcpu = (2*load)/(2*load+1)*estcpu + p_nice
  load assumed to be 1 here.
